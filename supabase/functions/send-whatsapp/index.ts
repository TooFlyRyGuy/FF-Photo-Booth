...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { userId, phoneNumber, imageUrl, imageId, eventId }: WhatsAppRequest = await req.json();

    if (!phoneNumber || !imageUrl) {
      return new Response(
        JSON.stringify({ success: false, error: 'Phone number and image URL are required' }),
    ...
    if (!settings.twilio_enabled || !settings.twilio_account_sid || !settings.twilio_auth_token || !settings.twilio_phone_number) {
      throw new Error('Twilio is not configured for WhatsApp');
    }
    ...
    const fromWhatsApp = `whatsapp:${settings.twilio_phone_number}`;
    ...
    if (!response.ok) {
      ...
      throw new Error(`Twilio WhatsApp API failed: ${errorText}`);
    }
